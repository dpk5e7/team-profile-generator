...
class TeamProfileGenerator {
   ...
   * If "test" is passed in, the application will use test data.
   * Otherwise, the application will prompt the user for data.
   * @param {string} value
   */
  start(value) {
    if (value === "test") {
  ...
  generateWithTestData() {
    ...
    const testEmployees = TestData.getTestEmployees();
  ...
  buildHTML(employees) {
    ...
    for (const employee of employees) {
      ...
      if (employee instanceof Manager) {
        ...
        uniqueItem = `<li class="list-group-item">Github: <a href="https://github.com/${employee.getGithub()}" class="card-link" target="_blank">${employee.getGithub()}</a></li>`;
